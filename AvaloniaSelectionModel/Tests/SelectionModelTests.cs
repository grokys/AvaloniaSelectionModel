...
namespace Avalonia.Controls.UnitTests
{
    public class SelectionModelTests
    {
        ...
        {
            SelectionModel<int> selectionModel = new SelectionModel<int>() { SingleSelect = true };
        ...
        {
            SelectionModel<int> selectionModel = new SelectionModel<int>();
            selectionModel.Source = Enumerable.Range(0, 10).ToList();

            int selectionChangedFiredCount = 0;
            selectionModel.SelectionChanged += delegate (object sender, SelectionModelSelectionChangedEventArgs<int> args)
            {
        ...
        public void ValidateCanSetSelectedIndex()
        {
            ...
            var ip = 34;//IndexPath.CreateFrom(34);
        ...
        //    if (handleChildrenRequested)
        //    {
        //        // Validate SourceIndexes.
        //        var expectedSourceIndexes = new List<IndexPath>()
        //        {
        //            Path(1),
        //            Path(1, 0),
        //            Path(1, 0, 1),
        ...
        //        Assert.Equal(expectedSourceIndexes.Count, sourcePaths.Count);
        //        for (int i = 0; i < expectedSourceIndexes.Count; i++)
        //        {
        //            Assert.True(AreEqual(expectedSourceIndexes[i], sourcePaths[i]));
        //        }
        //    }
        ...
        public void CanReplaceItem()
        {
            ...
            data[3] = 300;
            data[4] = 400;
            ValidateSelection(selectionModel, 5);
        }
        ...
        {
            var selectionModel = new SelectionModel<int>();
            _output.WriteLine("Set the source to 10 items");
            selectionModel.Source = Enumerable.Range(0, 10).ToList();

            bool selectedIndexChanged = false;
            bool selectedIndexesChanged = false;
            ...
            Assert.True(selectedIndexesChanged);
        ...
        {
            var target = new SelectionModel<int>();
            var raised = 0;
            ...
            target.SelectionChanged += (s, e) =>
            {
                ...
                Assert.Equal(new[] { 4 }, e.SelectedIndexes);
                Assert.Equal(new[] { 4 }, e.SelectedItems);
                ++raised;
            };
        ...
        {
            var target = new SelectionModel<int> { SingleSelect = true };
            var raised = 0;
            ...
            target.SelectionChanged += (s, e) =>
            {
                ...
                Assert.Equal(new[] { 3 }, e.DeselectedItems);
                Assert.Equal(new[] { 4 }, e.SelectedIndexes);
                Assert.Equal(new[] { 4 }, e.SelectedItems);
                ++raised;
            };
        ...
        //        Assert.Empty(e.DeselectedIndexes);
        //        Assert.Empty(e.DeselectedItems);
        //        Assert.Equal(new[] { new IndexPath(1, 1) }, e.SelectedIndexes);
        ...
        {
            var target = new SelectionModel<int> { SingleSelect = false };
            var raised = 0;
            ...
            target.SelectionChanged += (s, e) =>
            {
                ...
                Assert.Empty(e.DeselectedIndexes);
                Assert.Empty(e.DeselectedItems);
                Assert.Equal(expected, e.SelectedIndexes);
                Assert.Equal(expected, e.SelectedItems);
                ++raised;
            };
        ...
        //        Assert.Empty(e.DeselectedIndexes);
        //        Assert.Empty(e.DeselectedItems);
        //        Assert.Equal(expected.Select(x => new IndexPath(x / 10, x % 10)), e.SelectedIndexes);
        ...
        public void ClearSelection_Raises_SelectionChanged()
        {
            ...
            target.SelectionChanged += (s, e) =>
            {
                ...
                Assert.Equal(expected, e.DeselectedIndexes);
                Assert.Equal(expected, e.DeselectedItems);
                Assert.Empty(e.SelectedIndexes);
                Assert.Empty(e.SelectedItems);
                ++raised;
            };
        ...
        //    target.SelectionChanged += (s, e) =>
        //    {
        //        Assert.Equal(new[] { new IndexPath(1, 1) }, e.DeselectedIndexes);
        ...
        public void Setting_SelectedIndex_Raises_SelectionChanged()
        {
            ...
            target.SelectionChanged += (s, e) =>
            {
                Assert.Equal(new[] { 4, 5 }, e.DeselectedIndexes);
                Assert.Equal(new[] { 4, 5 }, e.DeselectedItems);
                Assert.Equal(new[] { 6 }, e.SelectedIndexes);
                Assert.Equal(new[] { 6 }, e.SelectedItems);
                ++raised;
            };
        ...
        public void Removing_Selected_Item_Raises_SelectionChanged()
        {
            ...
            {
                Assert.Empty(e.DeselectedIndexes);
                Assert.Equal(new[] { 4 }, e.DeselectedItems);
                Assert.Empty(e.SelectedIndexes);
                Assert.Empty(e.SelectedItems);
                ++raised;
            };
        ...
        [Fact]
        public void Removing_Unselected_Item_Before_Selected_Item_Raises_IndexesChanged()
        {
            var target = new SelectionModel<int>();
            var data = new ObservableCollection<int>(Enumerable.Range(0, 10));
            var raised = 0;
        ...
        [Fact]
        public void Removing_Unselected_Item_After_Selected_Item_Doesnt_Raise_IndexesChanged()
        {
            var target = new SelectionModel<int>();
            var data = new ObservableCollection<int>(Enumerable.Range(0, 10));
            var raised = 0;
        ...
        {
            var data = new[] { 1, 2, 3 };
            var target = new SelectionModel<int> { Source = data, SingleSelect = false };
        ...
        //    Assert.Equal(new[] { IndexPath.CreateFrom(1) }, target.SelectedIndexes);
        ...
        {
            var data = new[] { "foo", "bar", "baz" };
            var target = new SelectionModel<string> { Source = data, SingleSelect = false };
        ...
        //    Assert.Equal(new[] { new IndexPath(1), new IndexPath(2) }, target.SelectedIndexes);
        ...
        //            Assert.Empty(e.DeselectedIndexes);
        //            Assert.Empty(e.DeselectedItems);
        //            Assert.Equal(new[] { new IndexPath(1) }, e.SelectedIndexes);
        //            Assert.Empty(e.SelectedItems);
        //        }
        //        else if (raised == 1)
        //        {
        ...
        public void Can_Batch_Update()
        {
            ...
            target.SelectionChanged += (s, e) =>
            {
                ...
                Assert.Equal(new[] { 1 }, e.DeselectedItems);
                Assert.Equal(new[] { 4 }, e.SelectedIndexes);
                Assert.Equal(new[] { 4 }, e.SelectedItems);
                ++raised;
            };
        ...
        //    Assert.Equal(24, target.SelectedIndexes.Count);

        //    var Indexes = target.SelectedIndexes.ToList();
        //    var items = target.SelectedItems.ToList();

        //    target.SelectionChanged += (s, e) =>
        //    {
        //        Assert.Equal(Indexes, e.DeselectedIndexes);
        ...
        public void Batch_Update_Does_Not_Raise_PropertyChanged_Until_Operation_Finished()
        {
            ...
            var target = new SelectionModel<string> { Source = data };
            var raised = 0;
            ...
            Assert.Equal(3, target.AnchorIndex);
        ...
        //        Assert.Empty(e.DeselectedIndexes);
        //        Assert.Empty(e.DeselectedItems);
        //        Assert.Equal(new[] { new IndexPath(0) }, e.SelectedIndexes);
        ...
        //        if (raised == 0)
        //        {
        //            Assert.Equal(new[] { new IndexPath(0) }, e.DeselectedIndexes);
        ...
        //    target.SelectionChanged += (s, e) =>
        //    {
        //        Assert.Equal(new[] { new IndexPath(2) }, e.DeselectedIndexes);
        //        Assert.Equal(new[] { "baz" }, e.DeselectedItems);
        //        Assert.Equal(new[] { new IndexPath(0) }, e.SelectedIndexes);
        //        Assert.Equal(new[] { "foo" }, e.SelectedItems);
        //        ++raised;
        ...
        //    target.SelectionChanged += (s, e) =>
        //    {
        //        Assert.Equal(new[] { Path(0, 9) }, e.DeselectedIndexes);
        ...
        //    target.SelectionChanged += (s, e) =>
        //    {
        //        Assert.Equal(new[] { Path(0, 9, 1) }, e.DeselectedIndexes);
        //        Assert.Equal(new[] { selected }, e.DeselectedItems);
        //        Assert.Empty(e.SelectedIndexes);
        //        Assert.Empty(e.SelectedItems);
        //        ++raised;
        ...
        //    target.Select(2);
        //    target.Source = data;
        //    Assert.Equal(1, target.SelectedIndexes.Count);
        //    Assert.Equal(new IndexPath(2), target.SelectedIndexes.First());
        //}
        ...
        //        if (raised == 0)
        //        {
        //            Assert.Equal(new[] { Path(4) }, e.DeselectedIndexes);
        ...
        private void Select<T>(SelectionModel<T> manager, int index, bool select)
        {
        ...
        //private void Select<T>(SelectionModel<T> manager, int groupIndex, int itemIndex, bool select)
        //{
        ...
        //private void Select<T>(SelectionModel<T> manager, IndexPath index, bool select)
        //{
        //    _output.WriteLine((select ? "Selecting " : "DeSelecting ") + index);
        //    if (select)
        //    {
        ...
        //private void SelectRangeFromAnchor<T>(SelectionModel<T> manager, int index, bool select)
        //{
        ...
        //private void SelectRangeFromAnchor<T>(SelectionModel<T> manager, int groupIndex, int itemIndex, bool select)
        //{
        ...
        //private void SelectRangeFromAnchor<T>(SelectionModel<T> manager, IndexPath index, bool select)
        //{
        //    _output.WriteLine("SelectRangeFromAnchor " + index + " select: " + select.ToString());
        //    if (select)
        //    {
        ...
        private void ValidateSelection<T>(
            SelectionModel<T> selectionModel,
            params int[] expectedSelected)
        {
            Assert.Equal(expectedSelected, selectionModel.SelectedIndexes);
        }
        ...
        //                path.Add(i);
        //                var childPath = IndexPath.CreateFromIndexes(path);
        //                if (child != null)
        //                {
        ...
        static int[] Path(params int[] path)
        {
            return path;
        }
    ...
    class CustomSelectionModel : SelectionModel<int>
    {
